...
async function main() {
    try {
        ...
        // Create HTTPS server
        https.createServer(sslOptions, app).listen(config.faucet.port, () => {
            terminal.green(`Faucet listening on HTTPS port ${config.faucet.port}\n`)
        })
    } catch (error) {
...
// Set views directory to /home/traaitt/xte
app.set('views', '/home/traaitt/xte')
...
app.use('/src', Express.static(Path.join(__dirname, 'src'))) // Serve static files from XeniumFaucet/src
...
app.post('/claimCoins', async (req, res) => {
    ...
    try {
        ...
        const atomicsToSend = generateRawCoinsToBeSent(config.faucet.minimumCoinsToBeSent, config.faucet.maximumCoinsToBeSent);
